
// Oneboxing
pref('extensions.cv-pls.oneBox', true);
pref('extensions.cv-pls.oneBoxHeight', 30);
pref('extensions.cv-pls.removeCompletedOneboxes', true);

// Extra buttons in chat
pref('extensions.cv-pls.cvPlsButton', true);
pref('extensions.cv-pls.delvPlsButton', true);

// Vote request notification
pref('extensions.cv-pls.soundNotification', false);
pref('extensions.cv-pls.avatarNotification', true);
pref('extensions.cv-pls.removeLostNotifications', true);
pref('extensions.cv-pls.removeCompletedNotifications', true);
pref('extensions.cv-pls.desktopNotification', false);

// Vote request status
pref('extensions.cv-pls.showCloseStatus', true);
pref('extensions.cv-pls.pollCloseStatus', true);
pref('extensions.cv-pls.pollInterval', 5);
pref('extensions.cv-pls.strikethroughCompleted', true);

// Backlog
pref('extensions.cv-pls.backlogEnabled', false);
pref('extensions.cv-pls.backlogAmount', 5);
pref('extensions.cv-pls.backlogRefresh', true);
pref('extensions.cv-pls.backlogRefreshInterval', 60);
